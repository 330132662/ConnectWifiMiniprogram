...
util.request = function (option) {
  ...
  option.showLoading =
    typeof option.showLoading != 'undefined' ? option.showLoading : false
...
util.footer = function ($this) {
  ...
  for (let i in tabBar['list']) {
    tabBar['list'][i]['pageUrl'] = tabBar['list'][i]['pagePath'].replace(
      /(\?|#)[^"]*/g,
      ''
    )
  }
  ...
  if (emoji) {
    for (var i in emoji) {
      string = string.replace(
        emoji[i],
        '[U+' + emoji[i].codePointAt(0).toString(16).toUpperCase() + ']'
      )
    }
  }
  return string
